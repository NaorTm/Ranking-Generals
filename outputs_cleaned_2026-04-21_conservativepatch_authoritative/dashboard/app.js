(function () {
  ...
  const COLORS = {
    robust_elite: "#227c5d",
    strong_but_model_sensitive: "#c77d22",
    caution_likely_artifact: "#b74a4a",
    other_ranked: "#6f7a72",
    battle: "#2f6db3",
    operation: "#5b8c5a",
    campaign: "#b27c2f",
  ...
  const GROUP_LABELS = {
    robust_elite: "Robust elite",
    strong_but_model_sensitive: "Strong but model-sensitive",
    caution_likely_artifact: "Caution / likely artifact",
  ...
  const state = {
    modelKey: "baseline_conservative",
    metricKey: "rank",
    eraFilter: "all",
    pageTypeFilter: "all",
  ...
  function filteredCommanders() {
    return COMMANDERS.filter((commander) => {
      ...
      if (state.searchTerm) {
        const haystack = `${commander.name} ${commander.primaryEraBucket || ""} ${commander.interpretiveEra} ${commander.robustnessCategory}`
          .toLowerCase();
  ...
  function renderOverview() {
    ...
    const robustCount = cohort.filter((commander) => commander.robustnessCategory === "robust_elite").length;
    const cautionCount = cohort.filter((commander) => commander.robustnessCategory === "caution_likely_artifact").length;
    ...
    cardContainer.innerHTML = [
      ...
      },
      {
        ...
        detail: top ? `${formatRank(top.ranks[state.modelKey])} · score ${formatScore(top.scores[state.modelKey])}` : "No commander available",
      },
      {
        label: "Robust elite in filter",
        value: formatNumber(robustCount),
        detail: `${formatNumber(cautionCount)} caution cases remain in the filtered cohort`,
      },
      {
    ...
    document.getElementById("top-tier-columns").innerHTML = byGroup
      .map((entry) => {
        const description =
          entry.group === "robust_elite"
            ? "Most defensible cross-model leaders."
            : entry.group === "strong_but_model_sensitive"
              ? "Serious contenders whose placement moves materially by model."
              : "High-visibility cases that need stronger caution.";
        return `
          <div class="tier-column ${entry.group === "robust_elite" ? "robust" : entry.group === "strong_but_model_sensitive" ? "strong" : "caution"}">
    ...
    container.innerHTML = state.selectedIds
      .map((id) => {
        ...
        return `
          <span class="chip">
            ${commander.name}
            <button type="button" data-remove-id="${id}" aria-label="Remove ${commander.name}">×</button>
          </span>
        `;
      })
      .join("");
  ...
  function renderSensitivityChart() {
    ...
    const groups = ["robust_elite", "strong_but_model_sensitive", "caution_likely_artifact", "other_ranked"];
    const traces = groups
      .map((group) => {
  ...
  function renderComparisonCards() {
    ...
    container.innerHTML = set
      .map((commander) => {
        const flags = [...(commander.cautionFlags || []), ...(commander.featureQualityFlags || [])].slice(0, 8);
        return `
          <article class="comparison-card">
            ...
            <div class="muted">${commander.interpretiveReason || "No interpretive note available for this commander in the current classification layer."}</div>
